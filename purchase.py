...
def purchase(List):
    ...
    print("\nHello "+a_name+"! Welcome to Quickpark.\nLook above and select your choice.")
    q={}  #assign empty dictionary with variable name 'q'.
    car = input("Vehicle number")
    flag="Y"
    while flag.upper()=="Y":  #check and go if flag is 'Y' or 'y'.
        product=input("\nWhich slot do yo prefer? ")
        ...
        if product_name==L[0][0].upper() or product_name==L[1][0].upper() or product_name==L[2][0].upper(): #check the user entered product name with stock of store
            ...
            while p==True:
                try:
                    p_quantity=int(input("How many "+product+" do you want to park: "))
                    p=False
            ...
            flag=(input(a_name+" do you want park more vehicles?(Y/N)"))
        else:
            print("sorry!! "+product+" is not available right now.")
            print("\nChoose from following slots please!")
            print("--------------------------------------------")
    ...
    print ("\nYou Choosed vehicle and it's slot respectively:\n",q,"\n")
    '''
    ...
    for keys in q.keys():
        if keys==L[0][0].upper(): #executes this operation if product is phone entered by customer.
            ...
            print("\nTotal cost for 2 wheeler: ",p_amount)
    ...
    dis=0.0
    if ((f_amount>=30) and (f_amount<40)):
        discount=disc
        if(discount<=5.0):
    ...
    import datetime  #import system date and time for create a unique involve name.
    ...
    file=open(invoice+" ("+a_name+").txt","w")      #generate a unique involve name and open it in write mode.
    file.write("=============================================================")
    file.write("\nQUICKPARK\t\t\t\tINVOICE")
    ...
    for keys in q.keys():           #In this loop, write in a file only those product which is purchase by user.
        if keys=="2 wheeler":
            file.write(str("\n"+str(keys)+" \t\t "+str(q['2 wheeler'])+" \t\t "+str(L[0][1])+" \t\t "+str(p_amount)))
        elif keys=="3 wheeler":
            file.write(str("\n"+str(keys)+" \t\t "+str(q['3 wheeler'])+" \t\t "+str(L[1][1])+" \t\t "+str(l_amount)))
        elif keys=="4 wheeler":
            file.write(str("\n"+str(keys)+" \t\t "+str(q['4 wheeler'])+" \t\t "+str(L[2][1])+" \t\t "+str(h_amount)))
    ...
    file.write("\n-------------------------------------------------------------")
    file.write("\n\n\tThank You "+a_name+" for your bookings.\n\t\tSee you again!")
    file.write("\n=============================================================")
    file.close()
    return q
